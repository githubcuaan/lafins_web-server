...
function JarBox({
  ...
  const interactiveClasses =
    "cursor-pointer transform transition-transform duration-200 ease-out hover:scale-105 active:scale-100 hover:shadow-lg hover:ring-2 hover:ring-indigo-200 dark:hover:ring-indigo-500 focus:outline-none focus-visible:ring-4 focus-visible:ring-indigo-300";
  ...
  return (
    ...
    >
      <div className="shrink-0 w-12 h-12 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center">
        {icon}
      </div>

      <div className="flex-1 text-left">
        <div className="flex items-baseline gap-3">
          <div className="text-sm font-medium text-slate-700 dark:text-white whitespace-normal wrap-break-word">
            {name}
          </div>
          {typeof percentage === "number" && (
            <div className="text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
...
export function JarList({ className = "", jars = [] }: JarListProps) {
  ...
  const [selectedJarId, setSelectedJarId] = useState<number | string | null>(
    null,
  );
    ...
    <>
      <div className={containerClass}>
        {Array.isArray(jars) &&
          jars.map((j: Jar) => {
            const iconClass =
              iconByKey[j.key as string] ?? "fa-solid fa-circle-dot";
            // Using <i> for FontAwesome class names present in project
            const icon = (
              <i
                className={`${iconClass} text-slate-700 dark:text-white`}
                aria-hidden
              />
            );
            return (
              <JarBox
                ...
                icon={icon}
                onClick={() => openAddOutcomeModal(j.id)}
              />
            );
          })}
      </div>
