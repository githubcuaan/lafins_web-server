...
const buildChartData = (jars) => {
  const items = Array.isArray(jars)
    ? jars
    : typeof jars === "object" && jars !== null
      ? Object.keys(jars).map((k) => ({ label: k, value: jars[k] }))
      : [];
  ...
  const values = items.map(
    (it) => Number(it.balance ?? it.value ?? it.percentage ?? 0) || 0,
  );
...
const centerTextPlugin = {
  ...
  beforeDraw: (chart) => {
    ...
    const total =
      datasets[0]?.data?.reduce((s, v) => s + (Number(v) || 0), 0) ?? 0;
...
const options = {
  ...
  plugins: {
    ...
    legend: {
      position: "right",
      labels: { boxWidth: 10, padding: 16, color: undefined },
    },
    tooltip: {
      callbacks: {
...
// Main Component
export default function JarDistributionPie({ jars = [] }) {
  const chartData = buildChartData(jars);
  ...
  if (!okDatasets) {
    ...
    console.warn(
      "JarDistributionPie: chartData.datasets is invalid",
      chartData,
    );
    return (
  ...
  const responsiveOptions = useMemo(
    () => createResponsiveOptions(themeOptions, containerRef, 640),
    [size, measuredWidth, themeOptions],
  );

  return (
    <div
      ref={containerRef}
      className="w-full p-4 rounded-lg shadow-sm dark:shadow-none bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-gray-700"
    >
      <div className="flex-1 text-center ">
        <h3 className="text-m font-medium text-slate-700 dark:text-white mb-2">
          Jar Distribution
        </h3>
      </div>
