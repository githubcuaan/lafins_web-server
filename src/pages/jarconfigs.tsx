...
export default function Jarsconfig() {
    ...
    const { data: serverJars = [], isLoading, error } = useQuery({
        queryKey: ['jars'],
    ...
    const bulkUpdateMutation = useMutation({
        ...
        onSuccess: () => {
            setAlert({ message: 'Jar percentages saved successfully.', variant: 'success' });
            queryClient.invalidateQueries({ queryKey: ['jars'] });
        },
        onError: () => {
            setAlert({ message: 'Failed to save jar percentages.', variant: 'error' });
        },
    });
